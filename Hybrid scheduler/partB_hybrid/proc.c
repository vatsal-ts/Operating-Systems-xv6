...
void insert(struct proc *p,int var){
  if(var == 0){
    acquire(&pqueue.lock);
    if(pqueue.siz==NPROC){
      release(&pqueue.lock);
...
struct proc * extractMin(int var){
  if(var == 0){
    acquire(&pqueue.lock);
    if(pqueue.siz == 0){
      release(&pqueue.lock);
...
scheduler(void)
{
  ...
  for(;;){
    ...
    if(isEmpty(0)){
      ...
      while(!isEmpty(1)){
        if((p = extractMin(1)) == 0){release(&ptable.lock);break;}
        insert(p,0);
      }
    }
    label:
